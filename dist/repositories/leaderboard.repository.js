...
exports.leaderboardRepository = {
    ...
    getLeaderboardFromTrackUnfiltered: async (trackName) => {
        const [rows] = await leaderboard_database_config_1.pool.query("SELECT * FROM leaderboard_all_track_laps_unique WHERE track_name = ? and status = 'approved'", [trackName]);
        // Cast to a more specific type if you have one, otherwise 'any[]' is fine for now
        return rows;
    },
    getRiderLapsAll: async (riderName) => {
        const [rows] = await leaderboard_database_config_1.pool.query("SELECT * from rider_laptimes_all where rider_name = ? and status = 'approved'", [riderName]);
        return rows;
    }
};
