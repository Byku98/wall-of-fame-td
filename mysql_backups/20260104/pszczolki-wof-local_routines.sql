...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`%` SQL SECURITY DEFINER */
/*!50001 VIEW `riders_from_track` AS select `riders`.`rider_name` AS `rider_name`,`tracks`.`track_name` AS `track_name` from ((`laps` left join `riders` on((`laps`.`rider_id` = `riders`.`rider_id`))) left join `tracks` on((`laps`.`track_id` = `tracks`.`track_id`))) order by `riders`.`rider_name` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-01-04 15:01:53
